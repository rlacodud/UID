...
## __2. React의 특징__
### __(1) JSX 문법__
#### JSX는 __자바스크립트 안에서 HTML 문법을 사용해서 view를 구성할 수 있게__ 도와주는 자바스크립트 문법이다.
```jsx
...
```
#### 위 코드가 JSX 문법이다.
#### 보다시피 __HTML 문법과 자바스크립트 문법이 혼재__ 돼 있다는 걸 알 수 있다.
...
>#### 개인적으로 제일 잘 설명했다고 생각하는 유튜브 링크를 첨부한다. [Virtual DOM](https://www.youtube.com/watch?v=BYbgopx44vo)
...
#### [react_developer_tool](https://chrome.google.com/webstore/detail/react-developer-tools/fmkadmapgofadopljbjfkapdkoienihi?hl=en-US)에 접속해 React Developer Tools 확장 프로그램을 다운받고 React 페이지에서 개발자 도구를 키면 __상단에 Components라는 탭이 새로 생기고__ 아래와 같이 Component 단위별로 나눠서 확인이 가능해진다.
![tool][tool]
...
#### 우선 코드를 쳐보며 직접 배워나가고 마지막에 요약하여 정리하는 방식으로 진행하자.
...
#### 이어서 React를 다룰 때 가장 중요한 개념은 `props`와 `state`이다.
...
#### 그럼 이제 응용을 하며 추가적인 사항들을 배워나가면 되는데
#### 그 중에서 꼭 알아야 하는 게 __생명 주기(LifeCycle)이다.__
...
[lifecycle]: ./img/lifecycle.jpeg "lifecycle"  
...
npm run build
```
#### 이후에 디렉터리 구조를 보면 build라는 폴더가 생긴 걸 확인할 수 있다.
![build][build]
...
#### 위 과정들을 통해 실 서버 환경이 완성되었다.
#### 그럼 이제 아래 명령어를 통해 웹 서버를 통해 빌드된 앱을 실행해보자.
```jsx
npm install -g serve
```
>#### __여기서 `-g`란?__
