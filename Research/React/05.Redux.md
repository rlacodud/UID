...
## __1. Redux란?__
#### Redux는 자바스크립트 앱을 위한 예측 가능한 상태 저장소다.
...
####  `store`라는 하나뿐인 데이터 공간이 있다.

<br>

### __(2) State is read-only__
#### React에서 `setState` 메소드를 활용해야만 상태 변경이 가능하다.
#### Redux에서도 `action`이라는 객체를 통해서만 상태 변경이 가능하다.
...
>#### 실제 정보가 담겨있는 그릇
>#### __Reducer__
>#### Reducer는 현재 상태와 Action 객체를 받아 필요하다면 새로운 state를 return하는 함수이다.
```javascript
const initialState = {
 counter: 1,
}
...
>#### __- getState__
>#### state의 값을 가져와 render에게 전달한다.

<br>

#### __(1-2) Action__
#### 상태에 변화가 필요할 때 Action을 일으켜야 한다.
#### Action은 자바스크립트 객체 형식으로 되어있으며 `type` 필드를 반드시 가지고 있어야 한다.
```javascript
{
...
#### 즉, 기존 상태에서 `number`에 `DisplayNumber` Component를 통해 변경된 `number`값에 `AddNumber` Component를 통해 변경된(`action`) `size`가 적용되어 더해지는 것이다.
